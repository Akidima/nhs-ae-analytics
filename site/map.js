...
const BASE = {
  // Esri public raster tiles are used directly: no API key or watermark.
  dark: { base: FALLBACK, labels: null },
  light: { base: FALLBACK, labels: null }
};
function isLight() { return document.documentElement.classList.contains('light'); }
let labelLayer = null;
let baseLayer = L.tileLayer(BASE[isLight() ? 'light' : 'dark'].base, {
  subdomains: 'abcd', maxZoom: 9,
  attribution: 'Tiles &copy; Esri &mdash; Sources: Esri, Garmin, FAO, NOAA, USGS'
}).addTo(map);

// Keep the map usable if the public tile service has a transient failure.
// The accessible trust list remains the source of truth while it loads.
...
window.addEventListener('ae-theme', () => {
  ...
  if (labelLayer) labelLayer.setUrl(set.labels);
  applyMarkerStates();
});
